...
for i=1:I %PROBLEMS (100)
    ...
    [B,M,theta_0,beta_0,p] = RD_PROBS_2(maxM);
...
for j=1:J %SIMULATIONS PER PROBLEM (1000)
fprintf('PROBLEM %d, SIM %d:', i,j)
    ...
    % Apply KG
    [best_arm_KG, theta_max_KG, theta_est_KG] = KG_2(mu, theta_0, beta_0, sigma_w, B , p);
    o_cost_KG(i,j)=max(mu)-mu(best_arm_KG);
    fprintf(' KG')
    
    % Apply KGP
    [best_arm_KGP, theta_max_KGP, theta_est_KGP] = KGP_2(mu, theta_0, beta_0, sigma_w, B , p);
    o_cost_KGP(i,j)=max(mu)-mu(best_arm_KGP);
    fprintf(' KGP')  
    % Apply KGPT a=b a=100
    a=100;
    [best_arm_KGPT1, theta_max_KGPT1, theta_est_KGPT1] = KGPT_2(mu, theta_0, beta_0, sigma_w, B , p, a, a);
    o_cost_KGPT1(i,j)=max(mu)-mu(best_arm_KGPT1);
    fprintf(' KGPT100')      
    % Apply KGPT a=b a=10
    a=10;
    [best_arm_KGPT2, theta_max_KGPT2, theta_est_KGPT2] = KGPT_2(mu, theta_0, beta_0, sigma_w, B , p, a, a);
    o_cost_KGPT2(i,j)=max(mu)-mu(best_arm_KGPT2);
    fprintf(' KGPT10')     
    % Apply KGB
    [best_arm_KGB, theta_max_KGB, theta_est_KGB, bonus] = KGB_2(mu, theta_0, beta_0, sigma_w, B , p);
    o_cost_KGB(i,j)=max(mu)-mu(best_arm_KGB);
    %rec_bonus(i,j,1:length(bonus))=bonus;
    fprintf(' KGB and done\n')
end
end
%%
...
% aavg_bonus(1:maxM)=avg_bonus(1,1,1:maxM);
